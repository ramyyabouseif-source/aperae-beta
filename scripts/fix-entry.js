...
// Simple script to load the review function and run it
const entryPath = path.join(__dirname, 'test-entry.json');
const entry = JSON.parse(fs.readFileSync(entryPath, 'utf8'));

// Import the review function (we'll need to compile TypeScript first or use a different approach)
// For now, let's just run the normalization logic directly

async function main() {
  try {
    // Use dynamic import for the TypeScript module
    const { reviewAndNormalizeWineEntry } = await import('./review-and-normalize-wine.ts');
